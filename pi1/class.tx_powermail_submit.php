 ...
 *  Copyright notice
 *
 *  (c) 2007 Alexander Kellner, Mischa Heißmann <alexander.kellner@einpraegsam.net, typo3.2008
 * @heissmann.org>
 *  All rights reserved
 *
...
class tx_powermail_submit extends tslib_pibase {
	...
	function main($conf, $sessionfields, $cObj) {
		...
		if ($this->ok == 1) {
			$this->markerArray = $this->markers->GetMarkerArray($this->conf, $this->sessionfields, $this->cObj, 'thx');
		} // Fill markerArray
		...
		if (!$this->div->subpartsExists($this->tmpl)) {
			$this->content = $this->pi_getLL('error_templateNotFound', 'Template not found, check path to your powermail templates');
		}
	...
	function saveMail() {
			// Configuration
			// Do not enable saving mails by default
		$this->save_PID = 0;
	...
	function emailSettings() {
		...
		if ($this->cObj->data['tx_powermail_recipient']) { // If receivers are listed in field receiver
			...
			for ($i = 0, $emails = ''; $i < count($emailarray); $i++) { // one loop for every key
				if (t3lib_div::validEmail($emailarray[$i])) {
					$emails .= $emailarray[$i] . ', ';
				} // if current value is an email write to $emails
				else {
					$this->sendername .= $emailarray[$i] . ' ';
				} // if current value is no email, take it for sender name and write to $this->sendername
			}
			if ($emails) {
		...
		elseif ($this->conf['email.']['recipient_mail.']['email_query.']) { // If own select query is chosen
			...
			$res = $GLOBALS['TYPO3_DB']->sql_query($query); // mysql query
			if ($res !== FALSE && !empty($query)) { // If there is a result
	...
	function userName() {
		if ($this->cObj->data['tx_powermail_sendername']) { // if name of sender was defined in flexform
			...
			for ($i = 0; $i < count($fields); $i++) { // one loop for every selected field
				if ($this->sessiondata[$fields[$i]]) {
					$sendername .= $this->sessiondata[$fields[$i]] . ' ';
				} // add value from current field
			}
	...
	function clearCaptchaSession() {
		if (t3lib_extMgm::isLoaded('sr_freecap', 0) || t3lib_extMgm::isLoaded('captcha', 0)) { // if captcha or freecap is loaded
			...
			} // clear session of captcha
			if (isset($_SESSION['sr_freecap_attempts'])) {
				$_SESSION['sr_freecap_attempts'] = 0;
			} // clear session of sr_freecap
			if (isset($_SESSION['sr_freecap_word_hash'])) {
				$_SESSION['sr_freecap_word_hash'] = FALSE;
			} // clear session of sr_freecap
		}
	...
	function secQuery($string) {
		...
		if ($failure) {
			$failure = substr(trim($failure), 0, -1);
		} // delete last ,

		if ($error === 0) {
			return $string;
		} // return query if no error
		else { // if error
